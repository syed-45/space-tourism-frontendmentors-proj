  ...
  const currPage = segments[1] === "" ? "home" : segments[1]
    ...
    return (
        <header className="w-full h-20 max-sm:mt-4 lg:mt-9 flex items-center lg:gap-10 p-7 sm:px-0 lg:p-0 justify-between lg:justify-start">
          <Image alt="logo" src={'/assets/shared/logo.svg'} width={1000} height={1000} className="w-9 sm:mx-7 lg:mx-0" />
          <button
            className="sm:hidden"
          ...
          <nav 
          className={`starting:translate-x-40 starting:opacity-0 transition transition-discrete duration-400 ease-out absolute z-40 w-7/10 min-h-screen top-0 right-0 backdrop-blur-2xl flex flex-col gap-9 pl-6
